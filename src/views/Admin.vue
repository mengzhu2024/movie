...
<div>
  ...
  ></el-pagination>
  <el-dialog append-to-body :close-on-click-modal="false" :visible.sync="editDialogFlag" title="编辑管理员" width="600px">
  ...
  methods: {
    ...
    save() {
      this.$refs.saveForm.validate(res => {
        if (res) {
          this.$axios({
            url: this.saveForm.id ? '/api/admin/update': '/api/admin/add',
            method: 'post',
            data: this.saveForm
          }).then(res => {
            if (res.data.code === 0) {
    ...
    toDelete(row) {
      this.$confirm('确定删除吗?', '提示', {
        ...
        this.$axios({
          url: '/api/admin/delete?id=' + row.id,
          method: 'delete'
        }).then(res => {
          if (res.data.code === 0) {
